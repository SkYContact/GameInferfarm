...
全文与 README 出现的每个倍数，口径如下（**不同腿长/不同配置的数字不可直接
互换**）：

| 倍数 | 分子 | 分母 | 口径 |
|---|---|---|---|
| **42×**（谱系首尾） | 815.5（本框架单组 2 银行） | 19.4（python 串行） | 五级谱系同腿：1280 局/64 链，4 腿交替中位，stagger 默认 |
| **116-118×**（宣传主数字） | ≈2300（同卡 6 组×2 银行，w14，stagger=0） | 19.4-19.8（python 串行） | **跨腿长**：分子=8192 局持续速率，分母=1280 局腿 |
| **11.5-12.1×**（vs C++ 朴素） | ≈2300（同上） | 190-200.7（每链线程+逐次） | 同上跨腿长 |
| **2.66-2.82×**（多组拆分单项） | ≈2300 | 815.5 | 框架内单组→6 组，8192 局口径 |
| **7.5-8.7×**（产线参考） | 317-368 | 42 | **另一负载**：YGO 真模型（行宽 176.9KB 产线），非本基准 |

python/C++ 基线未在 8192 局口径重跑（腿长标准化纪律见文末）——跨腿长倍数
只用于量级叙事；严谨对拍以 42× 谱系行为准。

## 测量纪律与环境边界（诚实条款）

- 环境为**单台笔记本**（9955HX + 5070Ti Laptop）：未锁频，功耗墙/温控存在
  跑间漂移；实测跑间噪声带 ±10-13%。所有判决数字=**≥3 跑取中位+腿指纹
  全同**；A/B 对比=交替 ≥4 腿防热偏置。跨机复现请先自报噪声带再比数。
- **指纹逐位同证确定性，不证语义正确**（两侧同错指纹也同）：正确性靠三路
  独立对账——腿指纹一致 + 决策数全同 + 胜率 85.6% 对训练记录 85.9%（见下）。
