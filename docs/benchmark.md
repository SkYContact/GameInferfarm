...
- **模型**：4 层 CNN + 全局 FC，6.85M 参数，~90M MACs/决策（`own/opp` 两
  ...
  消融档：同 3 epoch **无增强**对照 111/320 = 34.7%
  （bench_data/noaug_ablation3.log 复现）——8 对称增强一项的实测价值。
...
**终版修正（同日深夜，用户连环追问逼出的两个真发现）：**
- **workers=14 > 16**：16 worker 在 16 物理核上超订（调度台/ORT 内线程/DML
  驱动都要吃饭）。w12-14 平台 3.5-3.7s，**w14 冠军：8192 局 3.50-3.64s（4 跑）
  = 2303 局/s**（w16 的 2167 再 +6%）。worker 扫描：w10 3.88 / w12 3.66 /
  w13 3.68 / w14 3.57 / w16 3.78。
- **核显终审（铁案）**：8192 长腿 slack 充足（fb4 串行地板 2.7s < 3.6s 墙）
  仍全负——share 0.1-0.5 × w12/14/15/16 全部 ≤纯 cuda；**w12（留 4 空闲物理核
  养 DML 驱动）对照组仍负**（3.99 vs 3.66）→ 排除纯 CPU 争抢解释，混编开销
  是调度机器+驱动管线结构性成本（甩 1.6% 的活省 0.06s，开销 +0.3s）。
  "哪怕负担 100 局也是提升"的直觉错在：**核显算得动（纯 dml 组 166 局/s），
  但塞进饱和管线的手续费比它的算力贵**。610M+此 CNN+此机=不可用，结案。
