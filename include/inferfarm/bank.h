...
struct BankConfig {
    ...
    int spin = 0;                // 调度台等待模式（FARM_BANK_SPIN）：
                                 // 0=关（cv.wait_for(0.1ms)——实为 1-1.3ms
                                 //   定时器量子，每决策吃两次=决策延迟链主项）
                                 // 1=纯自旋（有在飞/有填充期不进 cv 纯轮询；
                                 //   专核语义：忙时调度台核 100% 独烧换延迟，
                                 //   实测 srv-lat 11.5×、全链 13×）
                                 // 混合等待（EMA 预测 deadline）判死拆除——
                                 // 判决 17 全账。2 等同 1。
};
